...
function createMenuItem(nameIn, priceIn, categoryIn){
  let obj ={name:nameIn, price:priceIn, category:categoryIn,};
  return obj;
...
*/
let burgers = createMenuItem("Burgers", 5.50, "Lunch");
console.log(burgers);
console.log(createMenuItem("Grilled Cheese", 4.00, "Lunch"));
console.log(createMenuItem("Gyro Platter", 5.00, "Dinner"));
...
export const burger = {
  ...
  discount: function(type){
    let pmulti = 0;
    type.toUpperCase() === "teacher".toUpperCase() || type.toUpperCase() === "student".toLocaleUpperCase() ? pmulti = 0.75 : type.toUpperCase() === "public".toLowerCase() ? pmulti = 0.90 : null;
    return this.price * pmulti;
  }
}
...
for (let i = 0; i < reviews.length; i++){
  reviews[i].name === "Julius" ? console.log(reviews[i].feedback) : null;
}
...
*/
  ...
  reviews.push({name:"Mind you damn business",rating:0,feedback:"Nobody gets a perfect rating on my watch"});
...
for (let i = 0; i < reviews.length; i++){
  reviews[i].name === "Reyna" ? reviews[i].feedback = "this place is chill with really cool people, great for getting work done on weekdays" : null;
}
...
function getReviewByIndex(array, index) {
    let theReturn = "";
    index <= array.length ? theReturn = `${array[index].name} gave the restaurant a ${array[index].rating} star review, and their feedback was: ${array[index].feedback}`  : console.log(`${index} is outside of the array index range`);
    return theReturn;
}
